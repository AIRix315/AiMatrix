...
class StoryboardService {
     ...
     * @returns 分镜资产
     */
    async generateScript(projectId, sceneAssetPath, artStyle = '写实风格') {
        try {
            ...
            const videoScenes = await this.generator.generateVideoPrompts(scriptScenes, characters, scene, artStyle);
            ...
            const storyboardAsset = await this.assetHelper.createAsset({
                ...
                customFields: {
                    ...
                    imagePrompts: imageScenes.map((s) => s.prompt || JSON.stringify(s)),
     ...
     * @returns 分镜资产列表
     */
    async batchGenerateScripts(projectId, sceneAssetPaths, artStyle = '写实风格') {
        const storyboards = [];
        for (const sceneAssetPath of sceneAssetPaths) {
            const storyboard = await this.generateScript(projectId, sceneAssetPath, artStyle);
     ...
     * 获取相关章节（辅助方法）
     */
    async getRelatedChapter(projectId, chapterId) {
        if (!chapterId) {
            return {};
        }
        try {
