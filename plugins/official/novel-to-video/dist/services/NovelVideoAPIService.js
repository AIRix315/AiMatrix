...
exports.NovelVideoAPIService = void 0;
const path = __importStar(require("path"));
class NovelVideoAPIService {
    /**
     * 构造函数 - 通过PluginContext注入依赖
     */
    constructor(apiManager, context) {
    ...
    async generateSceneImage(projectId, sceneAssetPath) {
        try {
            ...
            // 3. 下载图片到项目目录
            const savePath = path.join(path.dirname(sceneAssetPath), `${sceneData?.sceneId}.png`);
    ...
    async generateCharacterImage(projectId, characterAssetPath) {
        try {
            ...
            // 3. 下载图片到项目目录
            const savePath = path.join(path.dirname(characterAssetPath), `${characterData?.characterId}.png`);
            const imagePath = await this.downloadImage(imageUrl, savePath);
            // 4. 更新角色资产元数据
     ...
     * @returns 生成的视频路径
     */
    async generateStoryboardVideo(projectId, storyboardAssetPath, onProgress) {
        try {
            // 1. 获取分镜资产元数据
            ...
            // 3. 下载视频到项目目录
            const savePath = path.join(path.dirname(storyboardAssetPath), `${storyboard.id}.mp4`);
     ...
     * @returns 生成的音频路径
     */
    async generateDialogueAudio(projectId, voiceoverAssetPath, voiceFilePath) {
        try {
            // 1. 获取配音资产元数据
     ...
     * 下载视频到本地
     */
    async downloadVideo(url, savePath) {
        try {
            const response = await fetch(url);
            if (!response.ok) {
...
exports.NovelVideoAPIService = NovelVideoAPIService;
