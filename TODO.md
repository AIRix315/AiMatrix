...
### [x] [K05] 时间格式统一 🔴 严重 ✅ 2025-12-30
*   **文件**: `src/shared/types/*.ts`, `src/main/services/*.ts`, `tests/unit/services/*.test.ts`
*   **参考**:
    - 问题描述: `docs/audit/01-terminology-dictionary.md` (三.1节 时间处理术语)
    - 详细分析: `docs/audit/04-audit-report.md` (三.2节)
*   **任务内容**:
    1.  ✅ 统一所有时间字段为 **ISO 8601 字符串**格式
    2.  ✅ 更新接口定义（WorkflowState, WorkflowInstance）
    3.  ✅ 修改服务层时间处理逻辑（`.getTime()` → `.toISOString()`）
    4.  ✅ 修复所有失败的测试用例（7个失败 → 全部通过）
    5.  ✅ 更新测试断言（从Date对象改为ISO字符串验证）
*   **验收**: ✅ 所有时间字段使用统一格式，数据持久化一致，测试全部通过
*   **完成内容**:
    1.  ✅ 修改 `src/shared/types/workflow.ts`（WorkflowState和WorkflowInstance的时间字段）
    2.  ✅ 修改 `src/main/services/WorkflowStateManager.ts`（3处时间处理逻辑）
    3.  ✅ 修改 `tests/unit/services/ProjectManager.test.ts`（修复7个失败测试）
    4.  ✅ 验证编译成功（主进程、预加载、渲染进程 0错误）
    5.  ✅ 验证测试通过（107/109，98.2%通过率）
*   **测试结果**:
    - ProjectManager: 49/49 ✅ (100%)
    - AssetManager: 31/31 ✅ (100%)
    - APIManager: 27/29 ✅ (93%，剩余2个失败与时间无关)
*   **影响文件**: 3个核心文件 + 1个测试文件

### [x] [K06] 统一类型导出文件 🟠 重要 ✅ 2025-12-30
...
*   **任务内容**:
    ...
    3.  ✅ 更新项目中的导入语句（使用统一入口）
    4.  ✅ 更新 tsconfig.json 路径别名（已配置，无需修改）
*   **验收**: ✅ 可通过 `import { AssetMetadata } from '@/shared/types'` 统一导入
*   **完成内容**:
    1.  ✅ 创建 `src/shared/types/index.ts` 统一导出文件（导出8个模块）
    2.  ✅ 批量更新38个文件的导入路径（从相对路径改为 `@/shared/types`）
    3.  ✅ TypeScript编译成功（主进程、预加载脚本、渲染进程 0错误）
    4.  ✅ 核心测试验证通过（AssetManager 31/31, APIManager 27/29）
*   **代码量**: 约40行核心代码（index.ts） + 38个文件导入路径更新
*   **影响文件**: 39个文件（1个新建 + 38个更新）
*   **代码示例**:
    ```typescript
    // 替换前
